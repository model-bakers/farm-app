...
class TestAnimal:
    def test_create_cow(self):
        ...
        cow = baker.make(
            Animal, specie=Animal.Species.COW
        )  # using make will persist the object
...
class TestAddress:
    ...
    def test_registration_document(self):
        ...
        assert abspath(address.registration_document.path) == abspath(
            "mock_file.txt"
        )  # name given by Model Bakery
...
class TestPictures:
    def test_create_picture_file(self):
        ...
        assert abspath(picture.image.path) == abspath(
            "mock_img.jpeg"
        )  # name given by Model Bakery
